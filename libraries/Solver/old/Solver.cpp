                ...
                const int southLEDPin, const int modeLEDPin)
{
    ...
    // the first node can be created on reset
...
/*
    if(m_adj[STARTING_INDEX][STARTING_INDEX] == NULL){
        m_curr = createNode(s);
        m_curr->printInformation();
        m_starting = false;
...
*/  
    ...
    for(int i = 0; i < m_adj[s].size(); ++i){
        if(m_solved == false){
            if(m_adj[s][i] != NULL && m_visited[m_adj[s][i]->getID()] == false){
...
void Solver::solve(){
    ...
    //std::cout << sizeof(m_adj) << "\n";
    //std::cout << m_index << "\n";
    if(m_index < MAX_MAZE_SIZE){    
        if(m_solved == false){
            if(m_visited[m_index] == false){
